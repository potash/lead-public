...
create table output.kids_extra as (
    ...
    tests as (
         select *, true as venous from output.tests where sample_type='V'
         UNION ALL
         select *, false as venous from output.tests
    ),
    
    tests_detected_over1 as (
        select kid_id, venous, address_id, date, bll, lab_id, sample_type, age,
        coalesce(lag(date) OVER (PARTITION BY kid_id,venous order by date asc), date) as last_date,
        coalesce(lag(bll) OVER (PARTITION BY kid_id,venous order by date asc), bll) as last_bll
    ...
    detected as (
        select kid_id, coalesce(sample_type = 'V', false) as venous,
    ...
    full outer join tests_detected_agg using (kid_id, venous)
    full outer join last_blls using (kid_id, venous)
    full outer join detected using (kid_id, venous)
);

alter table output.kids_extra add primary key (kid_id, venous);
