...
SELECT setval('original_building_id', (select max(orig_bldg_)::int+1 from input.buildings));

create table buildings.original_buildings as (
    select coalesce(nullif(orig_bldg_, 0), nextval('original_building_id')) as orig_bldg_, orig_bldg_ as orig_bldg_2, gid,
