...
select kid_id,
    referral_date,
    first_sample_date,
    CASE WHEN init_date < first_sample_date THEN init_date END as init_date,
    CASE WHEN init_date < first_sample_date THEN hazard_int END as hazard_int,
    CASE WHEN init_date < first_sample_date THEN hazard_ext END as hazard_ext,
    CASE WHEN comply_date < first_sample_date THEN comply_date END as comply_date,
    CASE WHEN closure_date < first_sample_date THEN closure_date END as closure_date,
    CASE WHEN closure_date < first_sample_date THEN closure_code END as closure_code
from discontinuity.addresses
join output.kids using (kid_id)
join output.investigations using (address_id)
where referral_date < first_sample_date and first_sample_address
)
...
select kid_id,
    ...
    bool_or(referral_date is not null and closure_date is null) as investigations_open,
    count(referral_date) - count(closure_date) as investigations_open_count,
